  ...
  return (
    <div className="flex-1 flex flex-col">
      <div className="border-b border-white/10 px-5 overflow-x-auto">
        <div className="flex gap-4 -mb-px min-w-max pr-2">
          ...
          <TabButton active={activeTab === 'leaks'} onClick={() => setActiveTab('leaks')}>
            Leaks
          </TabButton>
      ...
      {activeTab === 'overview' && (
        <OverviewTab stats={stats} storedHands={storedHands} setReviewHand={setReviewHand} />
      )}
...
function OverviewTab({
  ...
  return (
    <div className="flex-1 overflow-y-auto px-5 pb-6 space-y-6">
        ...
        <ChartCard title="Cumulative profit">
          <ResponsiveContainer width="100%" height={160}>
            <LineChart data={profitOverTime}>
              ...
              <XAxis dataKey="hand" tick={{ fontSize: 12, fill: '#8E8E93' }} />
        ...
        <ChartCard title="Profit by position">
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={positionData}>
              <XAxis dataKey="position" tick={{ fontSize: 12, fill: '#8E8E93' }} />
              <YAxis tick={{ fontSize: 12, fill: '#8E8E93' }} />
              <Tooltip
                contentStyle={{ background: '#1C1C1E', border: '1px solid #38383A', borderRadius: 12 }}
              />
      ...
      <div>
        <div className="offsuit-module px-4 py-2.5 mb-2">
          <h3 className="offsuit-section-title">Recent hands</h3>
        </div>
        <div className="space-y-2">
          {storedHands.slice(0, 10).map((h) => (
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3.5 offsuit-module hover:bg-surface-raised text-sm transition-colors"
            >
              <span className="text-white">
                #{h.handNumber} <span className="text-offsuit-grey">{h.heroPosition}</span>
              </span>
              <span className={`tabular-nums font-semibold ${resultColor(h.stackChange)}`}>
...
function StatCard({
  ...
  return (
    <div className="p-4 offsuit-module">
      <div className="offsuit-section-title">{label}</div>
      <div className={`text-2xl font-semibold tabular-nums tracking-tight mt-1.5 ${valueClass}`}>{value}</div>
    </div>
  );
}
...
function ChartCard({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="p-4 offsuit-module">
      <h3 className="offsuit-section-title mb-3">{title}</h3>
      {children}
    </div>
  );
}
