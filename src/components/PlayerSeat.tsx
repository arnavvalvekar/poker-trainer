  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`relative flex flex-col items-center gap-1 px-3 py-2.5 rounded-module text-center min-w-[80px] transition-all duration-500 ease-out shadow-card ${
            isActive
              ? 'bg-surface ring-2 ring-white/30 scale-105 animate-seat-active'
              : 'bg-surface scale-100'
        ...
        >
          ...
          <PlayerAvatar
            name={player.name}
            playerKey={player.id}
            size={player.isHero ? 'hero' : 'opponent'}
            active={isActive}
          />
          <div className="font-medium text-xs truncate max-w-[80px] text-white">{player.name}</div>
          <div className="text-white font-semibold text-[13px] tabular-nums tracking-tight transition-all duration-300">
            {formatMoney(player.stack)}
          </div>
          <div className="text-offsuit-grey text-xs">{player.position}</div>

          {player.allIn && (
            <div className="text-offsuit-grey text-xs font-semibold animate-fade-in">All in</div>
          )}
          {player.folded && (
            <div className="text-offsuit-grey text-xs">Folded</div>
          )}
        </div>
      </div>
    </div>
